...
public struct LineCustom: View {
    ...
    var gradient: GradientColor = GradientColor(start: Colors2.GradientPurple, end: Colors2.GradientNeonBlue)
    var index:Int = 0
    ...
    public var body: some View {
        ZStack {
            if(self.showFull && self.showBackground){
                self.closedPath
                    .fill(LinearGradient(gradient: Gradient(colors: [Colors2.GradientUpperBlue, .white]), startPoint: .bottom, endPoint: .top))
...
struct LineCustom_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            LineCustom(data: ChartData(points: [12,-230,10,54]), frame: .constant(geometry.frame(in: .local)), touchLocation: .constant(CGPoint(x: 100, y: 12)), showIndicator: .constant(true), minDataValue: .constant(nil), maxDataValue: .constant(nil))
